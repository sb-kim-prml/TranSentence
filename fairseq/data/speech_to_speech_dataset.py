...
@dataclass
class SpeechToSpeechInferenceDatasetItem(object):
    index: int
    label: str
    source: torch.Tensor
    target: Optional[torch.Tensor] = None
...
class SpeechToSpeechDatasetCreator(object):
    ...
    def _from_list_inference(
        ...
        has_multitask = multitask is not None and len(multitask.keys()) > 0
        dataset_cls = SpeechToSpeechInferenceDataset
...
class SemToSpeechDatasetCreator(object):
    # mandatory columns
    KEY_ID, KEY_SRC_AUDIO, KEY_SRC_N_FRAMES = "id", "src_audio", "src_n_frames"
    KEY_TGT_AUDIO, KEY_TGT_N_FRAMES = "tgt_audio", "tgt_n_frames"
    # optional columns
    ...
    def _from_list(
        ...
        has_multitask = multitask is not None and len(multitask.keys()) > 0
        dataset_cls = SpeechToSpeechMultitaskDataset if has_multitask else SemToSpeechDataset
    ...
    def _from_list_inference(
        ...
        has_multitask = multitask is not None and len(multitask.keys()) > 0
        dataset_cls = SemToSpeechInferenceDataset
...
class SemToSpeechDataset(SpeechToTextDataset):
    def __init__(
        self,
        split: str,
        is_train_split: bool,
    ...
    def pack_units(self, input: torch.Tensor) -> torch.Tensor:
        ...
        assert input.dim() == 1
        stacked_input = (
            rearrange(input[:-1], '(lp1 p2) -> (lp1) p2', p2=4) - offset
        )  # remove <eos>
        scale = [
            pow(vocab_size, self.n_frames_per_step - 1 - i)
            for i in range(self.n_frames_per_step)
        ...
        scale = torch.LongTensor(scale).squeeze(0)
        res = input.new((len(input) - 1) // self.n_frames_per_step + 1).fill_(input[-1])
        res[:-1] = ((stacked_input + offset) * scale).sum(dim=1)
        return res

    def get_source_sem(self, index: Union[int, List[int]]) -> torch.Tensor:
        sem = torch.load(self.audio_paths[index]).unsqueeze(0)
        return sem

    def __getitem__(self, index: int) -> SpeechToSpeechDatasetItem:
        source = self.get_source_sem(index)
    ...
    def collater(
        ...
        frames = _collate_frames([x.source for x in samples], False)
        # sort samples by descending number of frames
...
class SemToSpeechInferenceDataset(SpeechToTextDataset):
    def __init__(
        self,
        split: str,
        is_train_split: bool,
    ...
    def __getitem__(self, index: int) -> SpeechToSpeechInferenceDatasetItem:
        source = self.get_source_sem(index)
        ...
        return SpeechToSpeechInferenceDatasetItem(
            index=index,
    ...
    def _collate_target(self, samples: List[SpeechToSpeechInferenceDatasetItem]) -> torch.Tensor:
        if self.target_is_code:
            target = fairseq_data_utils.collate_tokens(
                [x.target for x in samples],
                self.tgt_dict.pad(),
    ...
    def collater(
        self, samples: List[SpeechToSpeechInferenceDatasetItem], return_order: bool = False
    ) -> Dict:
        if len(samples) == 0:
            return {}
        indices = torch.tensor([x.index for x in samples], dtype=torch.long)
...
class SpeechToSpeechInferenceDataset(SpeechToTextDataset):
    def __init__(
        self,
        split: str,
        is_train_split: bool,
...
class SpeechToSpeechInferenceDataset(SpeechToTextDataset):
    ...
    def collater(
        ...
        if return_order:
            out["order"] = order
>>>>>>> 153cde05cadebadfcd92aa8e2ad5249d9f7c19e5
        return out